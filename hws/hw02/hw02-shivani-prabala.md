...
``` r
data_frame_readr$Experience[data_frame_readr$Experience=="R"] <- 0 
data_frame_readr$Experience <- as.integer(data_frame_readr$Experience) 
str(data_frame_readr)
```
...
``` r
#histogram for EFF 
hist(data_frame_base$EFF, xlab = "EFF", main="History of Efficiency")
```
...
barplot(dfcor_dec$numbers, names.arg = dfcor_dec$names, main="Correlations between Player Stats and EFF", ylim=c(-1.0, 1.0), col=c("gray", "gray", "gray", "gray", "gray", "red", "red", "red"))
```
