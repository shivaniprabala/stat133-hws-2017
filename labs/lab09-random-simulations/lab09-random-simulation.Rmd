...
if (rand > 0.5) { 
  drawn_box1 <- sample(box1, 4, replace= TRUE)
} else {
  drawn_box2 <- sample(box2, 4, replace= FALSE)
  }
```
...
repetitions <- 1000
boxes <- c('box1', 'box2')
drawn_balls <- matrix(0, 1000, 4)

for (i in 1: repetitions){
  chosen_box <- sample(boxes, 1) #select a box 
  
  if (chosen_box== 'box1'){
    samp <- sample(box1, 4, replace= TRUE) 
    drawn_balls[i, 1] <- samp[1]
    drawn_balls[i, 2] <- samp[2]
    drawn_balls[i, 3] <- samp[3]
    drawn_balls[i, 4] <- samp[4]
  } else{
    samp <- sample(box2, 4, replace=FALSE)
    drawn_balls[i, 1] <- samp[1]
    drawn_balls[i, 2] <- samp[2]
    drawn_balls[i, 3] <- samp[3]
    drawn_balls[i, 4] <- samp[4]
...
```{r}
dat <- data.frame(drawn_balls)
ggplot(dat, aes(x=)) + geom
```
